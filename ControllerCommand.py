...
class CreateFolder(ControllerCommand):
    def __init__(self, path):
        super().__init__()
        self._message = ms.Message(Com_Type, ms.Class.Method, ms.Method.POST)
...
class RenameFile(ControllerCommand):
    def __init__(self, path, newName):
        super().__init__()
        self._message = ms.Message(Com_Type, ms.Class.Method, ms.Method.PUT)
        self._message.addOption(ms.Options.LOCATION_PATH, path)
...
class DeleteFile(ControllerCommand):
    def __init__(self, path):
        super().__init__()
        self._message = ms.Message(Com_Type, ms.Class.Method, ms.Method.DELETE)
        self._message.addOption(ms.Options.LOCATION_PATH, path)
...
class ListFolder(ControllerCommand):
    def __init__(self, path):
        super().__init__()
        self._message = ms.Message(Com_Type, ms.Class.Method, ms.Method.GET)
        self._message.addOption(ms.Options.LOCATION_PATH, path)
...
class GetMetadata(ControllerCommand):
    def __init__(self, path):
        super().__init__()
        self._message = ms.Message(Com_Type, ms.Class.Method, ms.Method.HEAD)
        self._message.addOption(ms.Options.LOCATION_PATH, path)
